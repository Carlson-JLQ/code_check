I’m checking the checker against the current analyzer API and will keep the existing class and frontend registration names intact. The fix is likely to stay in the AST-body checker model and remove the unsupported `BugReporter`/`Stmt` calls.I’ve got the shape of the repair: the current implementation is mixing AST traversal with path-sensitive reporting APIs that don’t exist in this Clang version. I’m rewriting it to use only the AST-body checker surface and keep the diagnostic logic localized.```cpp
...
class GeneratedNoElseBranchChecker : public Checker<check::ASTCodeBody> {
  ...
  static const IfStmt *getInnermostElseIfChainStart(const IfStmt *TopIf) {
  ...
  void checkStmtForIfChain(const Stmt *S, AnalysisManager &AM,
    ...
    const IfStmt *Innermost = getInnermostElseIfChainStart(IS);
    ...
    auto R = std::make_unique<PathSensitiveBugReport>(
        ...
        BR.getData());
    R->addRange(getViolationLocationStmt(IS)->getSourceRange());
