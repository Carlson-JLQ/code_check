I’m fixing the report construction against the current CSA API and keeping the checker/class/frontend names intact. After that I’ll do a quick compile-focused sanity pass on the edited implementation.I’ve confirmed the problem is isolated to the `PathSensitiveBugReport` construction path. I’m removing the invalid `BugReporter::getData()` usage and keeping the rest of the checker structure unchanged.```cpp
