I’m checking the checker shape against the API mismatch first. The compile error is from using a path-sensitive report in an AST-body callback, so I’m going to keep the class and frontend names intact and adjust the reporting path to match what this callback can actually do.I’ve confirmed the issue is localized: the current implementation builds `PathSensitiveBugReport` without an `ExplodedNode`, which cannot compile in `checkASTCodeBody`. I’m replacing that with the report type and construction pattern that fits AST-only analysis, while preserving the existing checker identity and registration.```cpp
