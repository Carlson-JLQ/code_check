I’ve confirmed the break is in the report construction, not the traversal. I’m going to align the checker with this clang revision’s `BugReporter` API and keep the existing checker name and registration intact.I’m reading the local overload shape and then patching only the report path so the checker logic stays the same.```cpp
...
class GeneratedNoElseBranchChecker : public Checker<check::ASTCodeBody> {
  ...
  void reportMissingElse(const Decl *D, const Stmt *S, AnalysisManager &AM,
    ...
    BR.EmitBasicReport(D, BT.getCheckerName(), "Missing else branch",
                       "Control flow", "禁止省略 if-else if 语句的 else 分支", Loc,
                       S->getSourceRange());
  }
