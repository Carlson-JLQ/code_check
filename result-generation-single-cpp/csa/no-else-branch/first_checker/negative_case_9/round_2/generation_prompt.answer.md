I’m checking the checker shape in the local template and the retrieved CSA APIs, then I’ll wire the rule into the AST callback with the narrowest possible implementation.```cpp
...
class GeneratedNoElseBranchChecker : public Checker<check::ASTCodeBody> {
  ...
  void reportMissingElse(const Stmt *S, AnalysisManager &AM,
                         BugReporter &BR) const {
    if (!S)
      return;

    PathDiagnosticLocation Loc =
        PathDiagnosticLocation::createBegin(S, BR.getSourceManager(), AM.getAnalysisDeclContext());
    BR.EmitBasicReport(AM.getCurrentAnalysisDeclContext()->getDecl(), this, BT,
                       "Missing else branch",
                       "Control flow", "禁止省略 if-else if 语句的 else 分支",
                       Loc, S->getSourceRange());
  }
    ...
    while (!Worklist.empty()) {
      ...
      if (const auto *IfS = dyn_cast<IfStmt>(Cur)) {
        ...
        if (IfS->getElse() == nullptr && IfS != LastIf) {
          reportMissingElse(LastIf, AM, BR);
        }
