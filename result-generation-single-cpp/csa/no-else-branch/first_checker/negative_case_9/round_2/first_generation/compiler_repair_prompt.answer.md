I’m updating the checker against the current Static Analyzer API and keeping the existing class and frontend names intact. The fix is narrow: use the `Decl *` already passed into `checkASTCodeBody` and stop calling the removed `AnalysisManager` accessor.I’ve confirmed the mismatch is only in the report path, not the traversal logic. I’m patching that call site now and leaving the rest of the plugin structure alone.```cpp
