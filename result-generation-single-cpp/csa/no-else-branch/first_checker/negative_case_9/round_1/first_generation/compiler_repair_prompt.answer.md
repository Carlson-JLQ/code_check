I’m checking the checker against the current Static Analyzer API and fixing the report construction so it compiles without changing the class or frontend names.```cpp
...
class GeneratedNoElseBranchChecker : public Checker<check::ASTCodeBody> {
  ...
  void reportMissingElse(const IfStmt *IS, AnalysisManager &AM,
    ...
    if (IS->getBeginLoc().isValid() && IS->getEndLoc().isValid())
