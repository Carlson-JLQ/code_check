I’m patching the checker in place, keeping the class and registration names intact, and removing the version-specific diagnostic call that fails to compile.```cpp
...
public:
  void checkASTCodeBody(const Decl *D, AnalysisManager &AM,
    ...
    auto R = std::make_unique<PathSensitiveBugReport>(
        *BT, "禁止省略 if-else if 语句的 else 分支", ViolatingIf->getIfLoc());
