I’m checking the checker’s report path against the current CSA APIs, then I’ll patch only the invalid plumbing and keep the existing class and frontend registration intact.The breakage is localized: `checkASTCodeBody` has the right hook, but the report is being built with an API that no longer exists in this tree. I’m switching that to a supported `PathSensitiveBugReport` construction and fixing the `AnalysisManager` call signature at the same time.```cpp
