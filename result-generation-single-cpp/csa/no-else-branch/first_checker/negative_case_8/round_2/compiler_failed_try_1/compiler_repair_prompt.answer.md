I’ve confirmed the breakage is API misuse, not a logic issue. I’m patching the checker to use the `checkASTCodeBody` declaration directly and to construct the bug report with a valid `ExplodedNode`-based overload, while keeping the class and registration names intact.```cpp
...
class GeneratedNoElseBranchChecker : public Checker<check::ASTCodeBody> {
  ...
  void reportMissingElse(const IfStmt *IfS, BugReporter &BR,
                         const Decl *D) const {
    if (!BT)
    ...
    PathDiagnosticLocation Loc = PathDiagnosticLocation::createBegin(
        IfS, BR.getSourceManager(), D);

    auto R = std::make_unique<PathSensitiveBugReport>(
        *BT, "禁止省略 if-else if 语句的 else 分支", nullptr, Loc, D);
    BR.emitReport(std::move(R));
  }

  void checkStmtForIfChain(const Stmt *S, BugReporter &BR,
                           const Decl *D) const {
    if (!S)
      return;
    ...
    if (const auto *IfS = dyn_cast<IfStmt>(S)) {
      ...
      if (hasElseIfChain(IfS) && !Outer->getElse())
        reportMissingElse(IfS, BR, D);
    }

    for (const Stmt *Child : S->children())
      checkStmtForIfChain(Child, BR, D);
  }
