I’ve got the failure mode: the checker is mixing AST-only traversal with path-sensitive reporting APIs. I’m switching it back to a plain AST-body checker and repairing the report construction so it matches the current Clang interfaces.```cpp
