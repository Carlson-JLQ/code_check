I’m checking the available AST and analyzer hooks, then I’ll wire the checker to walk `if` chains and report the first missing final `else` on chains that contain at least one `else if`.```cpp
