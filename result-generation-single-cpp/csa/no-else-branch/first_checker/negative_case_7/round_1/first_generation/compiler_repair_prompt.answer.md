I’m opening the checker and aligning it with the Clang analyzer APIs it actually uses, then I’ll patch the broken report construction and callback shape without changing the public class or frontend names.The current file is mixing declaration-level traversal with statement-level logic. I’m switching it to a decl-body walk, but only through valid statement APIs, and I’ll keep the registration string intact.```cpp
