I’m fixing the report construction to use a concrete analyzer report type and keeping the checker/frontend names unchanged. I’m also checking whether this AST-body callback can actually emit through `BugReporter` with the data it has, so the fix matches the callback shape instead of forcing a path-sensitive API onto it.I’ve got the likely failure point isolated: the checker is trying to instantiate `BugReport` directly, which is abstract. I’m patching that to a concrete report class and keeping the rest of the traversal intact.```cpp
