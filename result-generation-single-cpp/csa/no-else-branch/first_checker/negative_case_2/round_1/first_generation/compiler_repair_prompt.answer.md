I’m checking the checker against the current CSA reporting APIs and will patch only the report creation path, keeping the class and frontend registration names intact.The failure is exactly the old `BugReporter::createBasicReport`/raw pointer ownership pattern. I’m switching it to direct `BugReport` construction with `std::unique_ptr`, and I’m also verifying whether the report needs an exploded node in this AST-body checker.```cpp
