I’m checking the repository shape and the local checker patterns before I write the plugin, so I can stay aligned with the available Static Analyzer APIs and not invent unsupported hooks.```cpp
  ...
  void checkASTCodeBody(const Decl *D, AnalysisManager &AM,
    ...
    auto R = std::make_unique<PathSensitiveBugReport>(
        *BT, "禁止省略 if-else if 语句的 else 分支",
        BR.getBugReporter().getSourceManager().getLocForEndOfFile(
            BR.getBugReporter().getSourceManager().getMainFileID()));
