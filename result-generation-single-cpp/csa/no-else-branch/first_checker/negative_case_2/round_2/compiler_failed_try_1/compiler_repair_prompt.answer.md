I’ve got the mismatch: this checker is AST-based, but the current code is using path-sensitive report APIs that don’t exist in that context anymore. I’m switching it back to the template’s ASTCodeBody shape and using plain `BugReport` so it compiles against the current headers.```cpp
  ...
  void checkASTCodeBody(const Decl *D, AnalysisManager &AM,
    ...
    auto R = std::make_unique<BugReport>(
        *BT, "禁止省略 if-else if 语句的 else 分支",
        PathDiagnosticLocation(SM.getLocForEndOfFile(SM.getMainFileID()), SM));

    R->addRange(ReportedIf->getSourceRange());
