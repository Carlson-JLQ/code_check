I’m replacing the broken AST visitor and invalid bug-report plumbing with a direct AST walk that stays inside the APIs this checker actually has. Then I’ll recompile mentally against the template shape and keep the class and frontend registration names intact.I’ve got the failure mode pinned down: the current file mixes AST visitor scaffolding with path-sensitive reporting APIs that aren’t available in this callback. I’m switching it back to a plain `checkASTCodeBody` implementation and using `BugReporter` directly with a valid location from the `IfStmt`.```cpp
      ...
      if (HasElseIf && !Current->getElse()) {
        ...
        auto R = std::make_unique<BugReport>(
            ...
            BR.getSourceManager().getLocForStartOfFile(
                BR.getSourceManager().getMainFileID()));
