I’m checking the local checker pattern and then I’ll fill in the structural AST logic directly in the plugin file.I’ve got enough context to implement this as an AST walker over `IfStmt` chains, with the diagnostic anchored on the first `if` in the chain when the final `else` is missing. Next I’m writing the checker and keeping it self-contained in one C++ file.```cpp
