I’m checking the repo shape and then I’ll wire the checker into the provided plugin template with a structural AST walk focused on `if` chains that contain `else if` but no final `else`.I have enough context to implement this as an AST-based checker in the single plugin file. I’m keeping it local to the template’s allowed headers and will report at the last `else if` location when the final `else` is missing.```cpp
