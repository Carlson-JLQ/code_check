I’ve got the failure mode: the current file mixes AST-only traversal with path-sensitive reporting, and one of the AST assumptions is invalid in this Clang version. I’m editing it to stay within `ASTCodeBody` and use a plain `BugReport` with source ranges, which matches the checker shape and avoids the bad analyzer-node construction.```cpp
...
class GeneratedNoElseBranchChecker : public Checker<check::ASTCodeBody> {
  ...
  static const IfStmt *findOutermostIf(const IfStmt *IS) {
    ...
    while (true) {
      const IfStmt *Parent = nullptr;
      for (const Stmt *Child : Root->children()) {
        Parent = dyn_cast_or_null<IfStmt>(Child);
        if (Parent && Parent->getElse() == Root)
          break;
        Parent = nullptr;
      }
      if (!Parent)
  ...
  void reportMissingFinalElse(const IfStmt *Root, const IfStmt *LastElseIf,
    ...
    auto R = std::make_unique<BugReport>(
        *BT, "禁止省略 if-else if 语句的 else 分支", BeginLoc);
    R->addRange(BeginLoc, Root->getEndLoc());
