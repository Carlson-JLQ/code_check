I’m replacing the invalid graph-node plumbing with an AST-only report path that matches the checker’s current callback surface, and I’m keeping the class and frontend registration names intact.I’ve got the shape of the fix: keep the AST traversal, remove the unsupported `BugReporter` graph access, and emit a report without pretending we have a path node.```cpp
...
class GeneratedNoElseBranchChecker : public Checker<check::ASTCodeBody> {
  ...
  void emitReport(const IfStmt *IfS, const Decl *D, AnalysisManager &AM,
    ...
    auto R = std::make_unique<PathSensitiveBugReport>(
        *BT, "禁止省略 if-else if 语句的 else 分支", L, D);
