I’m patching the checker to use the current AST-body API correctly and to construct bug reports with an exploded node, which is what this Clang version expects.I’ve got the specific mismatch: this checker is using an old `PathDiagnosticLocation`-only report path. I’m switching it to the standard `CheckerContext` path-sensitive callback shape so it compiles against the current analyzer headers.```cpp
