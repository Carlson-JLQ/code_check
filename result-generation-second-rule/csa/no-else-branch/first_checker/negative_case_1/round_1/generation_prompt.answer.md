I’m checking the local shape of the checker template and the analyzer APIs it can lean on, then I’ll fill in the callback with a structural walk over `if` chains and keep the plugin self-contained.```cpp
#include <memory>
