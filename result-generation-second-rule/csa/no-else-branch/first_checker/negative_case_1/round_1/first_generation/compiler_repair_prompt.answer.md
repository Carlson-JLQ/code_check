I’m patching the checker in place and keeping the public-facing names intact. The fix is straightforward: remove the bogus parent-link lookup, tighten the AST walk, and match the constness of `AnalysisDeclContext`.```cpp
#include <memory>
...
class GeneratedNoElseBranchChecker : public Checker<check::ASTCodeBody> {
  ...
  void emitASTReport(const Stmt *Violation, const AnalysisDeclContext *ADC,
  ...
  void checkStmtForMissingElse(const Stmt *S, const AnalysisDeclContext *ADC,
                               BugReporter &BR) const {
