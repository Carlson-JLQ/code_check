I’m patching the checker to use the current Clang API shape and keeping the existing class and registration names intact. After that I’ll do a quick compile-oriented pass over the implementation for any other API mismatches.I’ve confirmed the reported failure is exactly the `isNullPointerConstant` call site. I’m tightening that helper so it no longer depends on a null `ASTContext`, and I’m leaving the traversal and diagnostic plumbing alone.```cpp
#include <memory>
#include <optional>
#include <utility>
