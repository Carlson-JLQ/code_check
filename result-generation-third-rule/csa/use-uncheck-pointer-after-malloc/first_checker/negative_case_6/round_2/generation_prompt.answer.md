I’m reading the available analyzer hooks and the existing template, then I’ll wire a path-sensitive checker that tracks allocation, null-check status, and first use per symbol with a single warning per variable.```cpp
#include <memory>
#include <optional>
#include <utility>
...
class GeneratedUseUncheckPointerAfterMallocChecker
  ...
  static bool isNullLikeExpr(const Expr *E) {
    ...
    if (E->isNullPointerConstant(nullptr, Expr::NPC_ValueDependentIsNotNull))
      return true;
    return false;
  }
