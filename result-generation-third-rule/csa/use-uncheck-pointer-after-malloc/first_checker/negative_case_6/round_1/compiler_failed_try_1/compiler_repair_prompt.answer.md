I’m fixing the AST API misuse directly in the checker and keeping the registration and naming intact. I’m also tightening the null-check recognition so the checker doesn’t suppress real violations by treating any comparison as a check.I’ve got the shape of the fix: replace the invalid `Expr::getASTContext()` use, keep the report path simple with `BasicBugReport`, and leave the rest of the standalone checker structure alone. After that I’ll verify the file is internally consistent with the template and compile-time expectations.```cpp
#include <memory>
#include <string>
  ...
  class Visitor : public RecursiveASTVisitor<Visitor> {
    ...
    static bool isNullLiteral(const Expr *E) {
      ...
      return isa<CXXNullPtrLiteralExpr>(E) ||
             ...
             (isa<CStyleCastExpr>(E) &&
              isNullLiteral(cast<CStyleCastExpr>(E)->getSubExpr())) ||
             (isa<ImplicitCastExpr>(E) &&
              isNullLiteral(cast<ImplicitCastExpr>(E)->getSubExpr()));
    }
    ...
    static bool isNullCheckExpr(const Expr *E, const VarDecl *VD) {
      ...
      if (const auto *BO = dyn_cast<BinaryOperator>(E)) {
        ...
        if (L == VD && isNullLiteral(BO->getRHS()))
          return true;
        if (R == VD && isNullLiteral(BO->getLHS()))
          return true;
      }
