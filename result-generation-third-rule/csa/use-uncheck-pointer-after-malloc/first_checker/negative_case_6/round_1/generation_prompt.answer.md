I’m tracing the existing checker shape first, then I’ll wire the rule into the AST callback with minimal dependencies and keep the diagnostic text exact.I have enough context to implement this directly in the plugin file. I’m using AST walk logic instead of path-sensitive state because the request is for a standalone checker and the available template is ASTCodeBody-based.```cpp
#include <memory>
#include <string>
