I’m patching the malformed `if` and keeping the checker’s public shape intact. After that I’ll do a quick compile-oriented sanity pass on the surrounding control flow so the fix stays minimal.The broken line is local to the allocation-assignment branch, so the repair is straightforward: split the declaration from the condition and keep the allocation handling unchanged.```cpp
#include <memory>
#include <utility>
  ...
  void handleStmt(const Stmt *S, AnalysisDeclContext *ADC, BugReporter &BR,
    ...
    if (const auto *BO = dyn_cast<BinaryOperator>(S)) {
      if (BO->isAssignmentOp()) {
        ...
        const auto *CE = dyn_cast<CallExpr>(ignoreParenCasts(BO->getRHS()));
        if (LHS && CE) {
