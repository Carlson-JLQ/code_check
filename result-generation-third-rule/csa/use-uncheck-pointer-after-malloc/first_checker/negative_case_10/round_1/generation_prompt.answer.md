I’m locating the smallest analyzer hooks that can model “allocated pointer must be checked before first use,” then I’ll wire the checker into the provided plugin template and keep the diagnostic text exact.I’ve got enough of the CSA surface to implement this as an AST/body walk with local state, which fits the standalone plugin constraint. Next I’m checking the repo only for syntax expectations around static analyzer plugin registration and any nearby checker patterns.```cpp
#include <memory>
