I’m fixing the compile breaks first, then I’ll tighten the checker back onto the existing AST-body pattern so it still preserves the requested class and frontend names.I’ve got the failure points isolated: one Clang API mismatch, one token split typo, and some cleanup needed around the standalone checker’s includes and report path. I’m applying the smallest edit set that gets it building without changing the registration surface.```cpp
#include <memory>
