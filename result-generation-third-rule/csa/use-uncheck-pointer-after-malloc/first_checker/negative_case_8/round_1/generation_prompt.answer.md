I’m checking the existing analyzer-facing APIs and then I’ll wire the checker into the AST path so it can track allocation, checks, and first use without touching LLVM sources.```cpp
#include <memory>
...
class GeneratedUseUncheckPointerAfterMallocChecker
  ...
  static bool isNullConstant(const Expr *E) {
    ...
    return E->isNullPointerConstant(
        ASTContext::NPC_ValueDependentIsNotNull);
  }
  ...
  static const Expr *getAssignmentRHS(const Stmt *S) {
    const auto *BO = dyn_cast_or_null<Binar yOperator>(S);
  ...
  static bool isNullCheckExpr(const Expr *E, const VarDecl *VD) {
    ...
    if (const auto *BO = dyn_cast<BinaryOperator>(E)) {
      ...
      return (LV == VD && isNullConstant(R)) || (RV == VD && isNullConstant(L));
    }
  ...
  void analyzeStmt(const Stmt *S, std::unordered_map<const VarDecl *, VarState> &States,
