I’m fixing the compile error first and keeping the checker’s public shape unchanged. Then I’ll tighten the null-check helper so it uses the AST context available in `checkASTCodeBody` instead of calling the Clang API with the wrong argument type.```cpp
#include <map>
#include <memory>
#include <optional>
#include <string>
...
class GeneratedUseUncheckPointerAfterMallocChecker
  ...
  static bool isNullLiteralOrMacro(const Expr *E, ASTContext &Ctx) {
    E = ignoreCasts(E);
    if (!E)
      return false;
  ...
  static bool isNullCheckCondition(const Expr *E, const VarDecl *VD,
    ...
    if (const auto *BO = dyn_cast<BinaryOperator>(E)) {
      ...
      return (exprUsesVar(L, VD) && isNullLiteralOrMacro(R, Ctx)) ||
             (exprUsesVar(R, VD) && isNullLiteralOrMacro(L, Ctx));
    }
  ...
  void analyzeStmt(const Stmt *S, AnalysisDeclContext *ADC, BugReporter &BR,
    ...
    if (const auto *IfS = dyn_cast<IfStmt>(S)) {
      if (const Expr *Cond = IfS->getCond()) {
        if (const auto *VD = getReferencedVar(Cond)) {
          if (isNullCheckCondition(Cond, VD, Ctx) && States.count(VD) &&
              States[VD].Allocated)
            States[VD].CheckedSinceAlloc = true;
        }
      }
      if (const Stmt *Then = IfS->getThen())
        analyzeStmt(Then, ADC, BR, States, Ctx);
      if (const Stmt *Else = IfS->getElse())
        analyzeStmt(Else, ADC, BR, States, Ctx);
      return;
    }
    ...
    if (const auto *WhileS = dyn_cast<WhileStmt>(S)) {
      ...
      if (const Stmt *Body = WhileS->getBody())
        analyzeStmt(Body, ADC, BR, States, Ctx);
      return;
    }

    if (const auto *ForS = dyn_cast<ForStmt>(S)) {
      if (const Stmt *Init = ForS->getInit())
        analyzeStmt(Init, ADC, BR, States, Ctx);
      ...
      if (const Stmt *Inc = ForS->getInc())
        analyzeStmt(Inc, ADC, BR, States, Ctx);
      return;
    }
    ...
    for (const Stmt *Child : S->children()) {
      if (const auto *ChildStmt = dyn_cast_or_null<Stmt>(Child))
        analyzeStmt(ChildStmt, ADC, BR, States, Ctx);
    }
  }
