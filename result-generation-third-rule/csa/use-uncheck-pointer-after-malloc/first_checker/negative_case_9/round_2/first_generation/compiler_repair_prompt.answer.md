I’m fixing the checker to the known-working CSA include surface and trimming it back to the template’s API usage. The current file is pulling a missing internal header, and there’s also a likely secondary compile issue from `std::map` without `<map>`.I’ve got the shape of the repair now: preserve the existing class and registration names, keep the ASTCodeBody frontend, and make the implementation compile against the public analyzer headers only. I’m applying the minimal include and code adjustments now.```cpp
#include <map>
#include <memory>
#include <optional>
#include <string>
