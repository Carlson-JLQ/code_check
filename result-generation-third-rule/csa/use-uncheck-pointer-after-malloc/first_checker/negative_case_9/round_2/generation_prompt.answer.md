I’m wiring the checker against the existing AST callback template and checking what syntax forms I can reliably recognize with the headers already in play. The main constraint here is to stay inside a single plugin file and keep the report anchored on the first real use after allocation.```cpp
#include <memory>
#include <optional>
#include <string>
#include <utility>
...
class GeneratedUseUncheckPointerAfterMallocChecker
  ...
  void analyzeStmt(const Stmt *S, AnalysisDeclContext *ADC, BugReporter &BR,
                   ...
                   const Expr *&PendingCondition, bool &InCondition) const {
    if (!S)
      return;
    ...
    if (const auto *IfS = dyn_cast<IfStmt>(S)) {
      ...
      InCondition = true;
      analyzeStmt(Cond, ADC, BR, States, PendingCondition, InCondition);
      InCondition = false;

      if (const Stmt *Then = IfS->getThen())
        analyzeStmt(Then, ADC, BR, States, PendingCondition, InCondition);
      if (const Stmt *Else = IfS->getElse())
        analyzeStmt(Else, ADC, BR, States, PendingCondition, InCondition);
      return;
    }

    if (const auto *WhileS = dyn_cast<WhileStmt>(S)) {
      const Expr *Cond = WhileS->getCond();
      PendingCondition = Cond;
      InCondition = true;
      analyzeStmt(Cond, ADC, BR, States, PendingCondition, InCondition);
      InCondition = false;
      if (const Stmt *Body = WhileS->getBody())
        analyzeStmt(Body, ADC, BR, States, PendingCondition, InCondition);
      return;
    }

    if (const auto *ForS = dyn_cast<ForStmt>(S)) {
      if (const Stmt *Init = ForS->getInit())
        analyzeStmt(Init, ADC, BR, States, PendingCondition, InCondition);
      ...
      if (const Stmt *Inc = ForS->getInc())
        analyzeStmt(Inc, ADC, BR, States, PendingCondition, InCondition);
      return;
    }
    ...
    if (const auto *E = dyn_cast<Expr>(S)) {
      if (const auto *UO = dyn_cast<UnaryOperator>(E)) {
        ...
        for (const Stmt *Child : E->children()) {
          if (const auto *CE = dyn_cast_or_null<Expr>(Child))
            analyzeStmt(CE, ADC, BR, States, PendingCondition, InCondition);
        }
      }

      if (InCondition) {
        for (auto &KV : States) {
          if (isNullCheckCondition(E, KV.first) && KV.second.Allocated)
            KV.second.CheckedSinceAlloc = true;
        }
      }
    }
    ...
    for (const Stmt *Child : S->children()) {
      if (const auto *ChildStmt = dyn_cast_or_null<Stmt>(Child))
        analyzeStmt(ChildStmt, ADC, BR, States, PendingCondition, InCondition);
    }
  }
...
public:
  void checkASTCodeBody(const Decl *D, AnalysisManager &AM,
    ...
    std::map<const VarDecl *, VarState> States;
    const Expr *PendingCondition = nullptr;
