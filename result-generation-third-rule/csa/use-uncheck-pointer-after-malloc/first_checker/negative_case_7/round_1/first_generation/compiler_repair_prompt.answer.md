I’m making the smallest compile fix first and keeping the checker’s public names intact. Then I’ll verify the resulting implementation still matches the standalone template shape.```cpp
#include <memory>
#include <optional>
#include <string>
...
class GeneratedUseUncheckPointerAfterMallocChecker
  ...
  void visitStmt(const Stmt *S, AnalysisDeclContext *ADC, BugReporter &BR,
    ...
    if (const auto *IF = dyn_cast<IfStmt>(S)) {
      const Expr *Cond = dyn_cast_or_null<Expr>(IF->getCond());
      if (Cond) {
        for (auto &It : States) {
