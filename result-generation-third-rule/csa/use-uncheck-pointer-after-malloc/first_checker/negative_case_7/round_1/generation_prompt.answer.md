I’m checking the checker shape first, then I’ll wire the allocation/use tracking into the existing AST callback style and keep the implementation self-contained in one C++ file.I have the available analyzer APIs and the template pattern. Next I’m implementing a conservative AST-based tracker that catches the required allocation-before-check-before-use order without depending on unlisted headers or checker-table edits.```cpp
#include <memory>
#include <optional>
#include <string>
