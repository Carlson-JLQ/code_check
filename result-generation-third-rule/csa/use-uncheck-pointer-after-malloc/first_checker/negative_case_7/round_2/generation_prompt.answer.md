I’m working from the provided checker template and the CSA APIs in context, and I’m keeping this as a single-file plugin as requested. I’m first mapping the rule to AST-level tracking so the implementation stays within the allowed headers and doesn’t depend on LLVM source edits.```cpp
#include <memory>
