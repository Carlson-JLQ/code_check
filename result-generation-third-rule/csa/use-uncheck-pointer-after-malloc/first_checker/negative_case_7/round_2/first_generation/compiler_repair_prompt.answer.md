I’m checking the checker against the local Clang API shape and then I’ll patch only the API mismatches, keeping the class and registration names intact.I’ve got the error sources: one bad Expr cast-stripper name and one null-constant helper call using the wrong signature. I’m applying the smallest edit that restores compilation and preserves the existing ASTCodeBody-based structure.```cpp
#include <memory>
...
class GeneratedUseUncheckPointerAfterMallocChecker
  ...
  static bool isCheckedConditionForVar(const Expr *Cond, const VarDecl *VD,
    ...
    if (const auto *UO = dyn_cast<UnaryOperator>(Cond)) {
      if (UO->getOpcode() == UO_LNot)
        return isCheckedConditionForVar(UO->getSubExpr(), VD, Ctx);
    }
    ...
    if (const auto *BO = dyn_cast<BinaryOperator>(Cond)) {
      ...
      if (LV == VD && isNullPointerConstantExpr(R, Ctx))
        return true;
      if (RV == VD && isNullPointerConstantExpr(L, Ctx))
        return true;
      return false;
    }
  ...
  void scanExprForUses(const Expr *E, llvm::DenseMap<const VarDecl *, VarInfo> &Vars,
                       ...
                       ASTContext &Ctx, bool InCondition = false) const {
    if (!E)
      return;
    ...
    if (const auto *UO = dyn_cast<UnaryOperator>(E)) {
      if (UO->getOpcode() == UO_LNot) {
        ...
        scanExprForUses(Sub, Vars, ADC, BR, Ctx, true);
        return;
      }
      scanExprForUses(UO->getSubExpr(), Vars, ADC, BR, Ctx, InCondition);
      return;
    }
    ...
    if (const auto *BO = dyn_cast<BinaryOperator>(E)) {
      if (BO->isComparisonOp()) {
        ...
        if (const VarDecl *VD = getReferencedVar(L)) {
          auto It = Vars.find(VD);
          if (It != Vars.end() && It->second.IsDynamic &&
              isNullPointerConstantExpr(R, Ctx)) {
            It->second.CheckedSinceAlloc = true;
            return;
          }
        }
        if (const VarDecl *VD = getReferencedVar(R)) {
          auto It = Vars.find(VD);
          if (It != Vars.end() && It->second.IsDynamic &&
              isNullPointerConstantExpr(L, Ctx)) {
            It->second.CheckedSinceAlloc = true;
            return;
          }
        }
      }
      scanExprForUses(BO->getLHS(), Vars, ADC, BR, Ctx, InCondition);
      scanExprForUses(BO->getRHS(), Vars, ADC, BR, Ctx, InCondition);
      return;
    }

    if (const auto *CE = dyn_cast<CallExpr>(E)) {
      for (const Expr *Arg : CE->arguments())
        scanExprForUses(Arg, Vars, ADC, BR, Ctx, InCondition);
      return;
    }
    ...
    if (const auto *ASE = dyn_cast<ArraySubscriptExpr>(E)) {
      ...
      scanExprForUses(ASE->getBase(), Vars, ADC, BR, Ctx, InCondition);
      scanExprForUses(ASE->getIdx(), Vars, ADC, BR, Ctx, InCondition);
      return;
    }
    ...
    for (const Stmt *Child : E->children()) {
      if (const Expr *ChildExpr = dyn_cast_or_null<Expr>(Child))
        scanExprForUses(ChildExpr, Vars, ADC, BR, Ctx, InCondition);
    }
  }
  ...
  void processStmt(const Stmt *S, llvm::DenseMap<const VarDecl *, VarInfo> &Vars,
    ...
    if (const auto *DS = dyn_cast<DeclStmt>(S)) {
      for (const Decl *D : DS->decls()) {
        ...
        if (const Expr *Init = getInitializerExpr(VD)) {
          scanExprForUses(Init, Vars, ADC, BR, Ctx);
    ...
    if (const auto *BO = dyn_cast<BinaryOperator>(S)) {
      if (BO->isAssignmentOp()) {
        ...
        scanExprForUses(RHS, Vars, ADC, BR, Ctx);
        if (VD) {
          auto &VI = Vars[VD];
          VI.VD = VD;
      ...
      scanExprForUses(BO->getLHS(), Vars, ADC, BR, Ctx);
      scanExprForUses(BO->getRHS(), Vars, ADC, BR, Ctx);
      return;
    }

    if (const auto *IS = dyn_cast<IfStmt>(S)) {
      scanExprForUses(IS->getCond(), Vars, ADC, BR, Ctx, true);
      for (const Stmt *Child : IS->children()) {
        if (Child != IS->getCond())
          processStmt(Child, Vars, ADC, BR, Ctx);
      }
      return;
    }

    if (const auto *WS = dyn_cast<WhileStmt>(S)) {
      scanExprForUses(WS->getCond(), Vars, ADC, BR, Ctx, true);
      processStmt(WS->getBody(), Vars, ADC, BR, Ctx);
      return;
    }

    if (const auto *FS = dyn_cast<ForStmt>(S)) {
      if (const Stmt *Init = FS->getInit())
        processStmt(Init, Vars, ADC, BR, Ctx);
      scanExprForUses(FS->getCond(), Vars, ADC, BR, Ctx, true);
      if (const Stmt *Inc = FS->getInc())
        processStmt(Inc, Vars, ADC, BR, Ctx);
      processStmt(FS->getBody(), Vars, ADC, BR, Ctx);
      return;
    }

    if (const auto *RS = dyn_cast<ReturnStmt>(S)) {
      scanExprForUses(RS->getRetValue(), Vars, ADC, BR, Ctx);
      return;
    }

    if (const auto *E = dyn_cast<Expr>(S)) {
      scanExprForUses(E, Vars, ADC, BR, Ctx);
      return;
    }
...
public:
  void checkASTCodeBody(const Decl *D, AnalysisManager &AM,
    ...
    processStmt(FD->getBody(), Vars, ADC, BR, AM.getASTContext());
  }
};
