I’m going to inspect the existing workspace shape for any nearby checker patterns, then I’ll wire the rule into the provided plugin template and keep the implementation self-contained.I’ve got the rule semantics and the required diagnostic text. Next I’m checking the local tree for any analyzer plugin conventions that matter here, especially how this workspace usually registers checkers and formats diagnostics.```cpp
#include <memory>
