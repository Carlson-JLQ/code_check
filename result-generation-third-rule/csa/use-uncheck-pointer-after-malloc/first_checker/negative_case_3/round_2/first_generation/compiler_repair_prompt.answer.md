I’m tightening the traversal to the template shape and fixing the compile errors at the same time. The main risk here is the AST walker being more complicated than this standalone checker needs, so I’m simplifying the type mismatches instead of trying to preserve the broken helper structure.```cpp
#include <memory>
