I’m patching the null-literal helper only, keeping the checker class and registry names intact, then I’ll sanity-check the resulting file shape against the provided template.```cpp
#include <memory>
...
class GeneratedUseUncheckPointerAfterMallocChecker
  ...
  void scanStmt(const Stmt *S, std::vector<VarInfo> &Vars,
                AnalysisDeclContext *ADC, BugReporter &BR) const {
    if (!S)
      return;
