I’m fixing the compile errors first, then I’ll trim the checker back toward the template so it stays within the expected CSA API surface and preserves the existing class and registration names.I’ve got enough context to patch this directly. The main issues are the bad `SourceLocation` API use and the missing `Decl *` passed into `getAnalysisDeclContext`, and I’m also removing a couple of dead branches that don’t belong in the standalone checker.```cpp
#include <memory>
#include <optional>
#include <string>
